...
export async function createApp(config: AppConfig, storeOverride?: Store, scheduleBackground?: BackgroundScheduler) {
  ...
  app.disable("x-powered-by"); app.use(helmet.default({ crossOriginResourcePolicy: false })); app.use(cors({ origin: config.clientOrigin, credentials: false })); app.use(express.json({ limit: "32kb" }));
