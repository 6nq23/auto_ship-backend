...
describe("AutoShip API", () => {
  ...
  it("generates a separate label containing only pickup-scheduled shipments", async () => { const store = new MemoryStore(); const app = await createApp(config, store); const token = await login(app); await store.addBatch({ batchId: "pickup-batch", createdAt: new Date().toISOString(), shippedBy: "admin", shipped: [{ orderNumber: "#RBD1", orderId: "ORD-NORMAL", awb: "AWB-1", courier: "Courier", cost: 10 }, { orderNumber: "#RBD2", orderId: "ORD-PICKUP", awb: "AWB-2", courier: "Courier", cost: 10, alreadyBooked: true, warningCode: "PICKUP_ALREADY_SCHEDULED", warning: "Already pickup scheduled" }], failed: [], labelUrl: "/demo-labels?ids=ORD-NORMAL%2CORD-PICKUP", totalShipped: 2, totalFailed: 0, demoMode: true }); const label = await request(app).post("/api/history/pickup-batch/label").set("Authorization", `Bearer ${token}`).send({ scope: "pickup_scheduled" }); expect(label.status).toBe(200); expect(decodeURIComponent(label.body.labelUrl)).toContain("ids=ORD-PICKUP"); expect(decodeURIComponent(label.body.labelUrl)).not.toContain("ORD-NORMAL"); });
  ...
  it("finishes a 40-plus order job across durable chunks", async () => { const app = await makeApp(); const token = await login(app); const orderNumbers = Array.from({ length: 41 }, (_, index) => `RBD${5001 + index}`); const started = await request(app).post("/api/shipping-jobs").set("Authorization", `Bearer ${token}`).send({ orderNumbers }); expect(started.status).toBe(202); const job = await waitForJob(app, token, started.body.job.jobId); expect(job.status).toBe("completed"); expect(job.processed).toBe(41); expect(job.shipped.length + job.failed.length).toBe(41); expect(job.logs.filter((log) => log.message.includes("Saved progress at")).length).toBeGreaterThanOrEqual(4); expect(job.result?.totalShipped).toBe(job.shipped.length); });
