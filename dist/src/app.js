...
export async function createApp(config, storeOverride, scheduleBackground) {
    ...
    const appendLog = (job, level, message, orderNumber) => {
        const at = new Date().toISOString();
        job.updatedAt = at;
        job.logs.push({ at, level, message, ...(orderNumber ? { orderNumber } : {}) });
        const prefix = `[shipping:${job.jobId}]${orderNumber ? ` [${orderNumber}]` : ""}`;
        (level === "error" ? console.error : console.info)(`${prefix} ${message}`);
    };
    const processJob = async (jobId) => {
        ...
        let job;
        try {
            job = await store.claimShippingJob(jobId);
            ...
            const recordProgress = async (event) => {
                ...
                if (event.type === "started")
                    appendLog(job, "info", "Checking the order and contacting NimbusPost.", event.orderNumber);
                if (event.type === "courier_attempt")
                    appendLog(job, "info", `Priority ${event.priority}/${event.total}: trying ${event.courierName} (courier ${event.courierId}, role ${event.roleId}).`, event.orderNumber);
                if (event.type === "courier_rejected")
                    appendLog(job, "error", `Priority ${event.priority} ${event.courierName} rejected: [${event.code}] ${event.error}`, event.orderNumber);
                ...
                if (event.type === "failed") {
                    ...
                    job.processed = job.shipped.length + job.failed.length;
                    appendLog(job, "error", `[${event.item.code}] ${event.item.error}`, event.orderNumber);
                }
                if (event.type === "labels_started")
                    appendLog(job, "info", `Generating labels for ${event.count} shipped order${event.count === 1 ? "" : "s"}.`);
                if (event.type === "labels_ready") {
                    job.labelUrl = event.labelUrl;
            ...
            if (chunk.length)
                await nimbus.shipMany(chunk, 5, recordProgress, false);
            await persistence;
            job.processed = job.shipped.length + job.failed.length;
            ...
            if (job.shipped.length) {
                ...
                if (pickupRecovered.length) {
                    try {
                        job.pickupScheduledLabelUrl = await nimbus.generateLabels(pickupRecovered.map((item) => item.orderId));
                    }
                    catch (error) {
                        appendLog(job, "error", `Pickup-status label generation failed: ${error instanceof Error ? error.message : "NimbusPost label request failed"}`);
                    }
                }
            }
            ...
            if (job) {
                job.status = "failed";
                job.error = error instanceof Error ? error.message : "Unexpected shipping error";
                appendLog(job, "error", `Shipment stopped because of a server error: ${job.error}`);
                await store.updateShippingJob(job).catch(console.error);
            }
    ...
    app.post("/api/auth/register", authLimiter, async (request, response, next) => {
        try {
            ...
            if (!/^[A-Za-z0-9._-]{3,32}$/.test(username))
                return response.status(400).json({ error: "Use 3–32 letters, numbers, dots, dashes, or underscores for the username." });
            if (password.length < 15 || Buffer.byteLength(password, "utf8") > 72)
                return response.status(400).json({ error: "Use a password between 15 and 72 characters." });
            const user = await store.createUser(username, await bcrypt.hash(password, 12), "packer");
            if (!user)
                return response.status(400).json({ error: "Account could not be created. Try a different username." });
    ...
    app.get("/api/auth/me", authenticate, (request, response) => response.json({ user: request.auth, demoMode: config.mockMode }));
    app.post("/api/shipping-jobs", authenticate, async (request, response, next) => {
        try {
            const orderNumbers = orderNumbersFrom(request.body);
            if (!orderNumbers || !orderNumbers.length || orderNumbers.length > 100 || orderNumbers.some((value) => !/^#RBD\d+$/.test(value)))
                return response.status(400).json({ error: "Send 1–100 unique order numbers in the #RBD1234 format." });
            const active = await store.getActiveShippingJob(request.auth.username);
            if (active)
            ...
            runInBackground(processJob(job.jobId));
        }
        catch (error) {
            next(error);
        }
    });
    app.get("/api/shipping-jobs/active", authenticate, async (request, response, next) => { try {
        const job = await store.getActiveShippingJob(request.auth.username) || null;
        response.json({ job });
    ...
    app.get("/api/shipping-jobs/:jobId", authenticate, async (request, response, next) => {
        try {
            const job = await store.getShippingJob(String(request.params.jobId));
            if (!job)
                return response.status(404).json({ error: "Shipment job was not found." });
            if (request.auth.role !== "admin" && job.createdBy.toLowerCase() !== request.auth.username.toLowerCase())
                return response.status(403).json({ error: "You cannot view this shipment job." });
            response.json({ job });
    ...
    app.post("/api/ship-bulk", authenticate, async (request, response, next) => {
        try {
            const orderNumbers = orderNumbersFrom(request.body);
            if (!orderNumbers || !orderNumbers.length || orderNumbers.length > 100 || orderNumbers.some((value) => !/^#RBD\d+$/.test(value)))
                return response.status(400).json({ error: "Send 1–100 unique order numbers in the #RBD1234 format." });
            const result = await nimbus.shipMany(orderNumbers);
            const batch = { ...result, batchId: crypto.randomUUID(), createdAt: new Date().toISOString(), shippedBy: request.auth.username, totalShipped: result.shipped.length, totalFailed: result.failed.length, demoMode: config.mockMode };
            await store.addBatch(batch);
            response.json(batch);
        }
        catch (error) {
    ...
    app.get("/api/history", authenticate, async (_request, response, next) => { try {
        response.json({ batches: await store.getHistory() });
    }
    catch (error) {
        next(error);
    } });
    app.post("/api/history/:batchId/label", authenticate, async (request, response, next) => {
        try {
            ...
            if (!shipments.length)
                return response.status(400).json({ error: pickupOnly ? "This batch has no pickup-pending or pickup-scheduled shipments to print." : "This batch has no successful shipments to print." });
    ...
    app.use((error, _request, response, _next) => { console.error(error); response.status(500).json({ error: "The request could not be completed. Please try again." }); });
