...
export function loadConfig() {
    ...
    const whatsappProvider = (["disabled", "meta", "whapi", "getgabs"].includes(requestedWhatsAppProvider) ? requestedWhatsAppProvider : "disabled");
    const requestedAiProvider = process.env.AI_PRIMARY_PROVIDER || "gemini";
    const aiPrimaryProvider = (["gemini", "claude", "openai"].includes(requestedAiProvider) ? requestedAiProvider : "gemini");
    ...
    if (!mockMode && (!process.env.NIMBUS_API_KEY || !process.env.NIMBUS_API_SECRET))
        throw new Error("NIMBUS_API_KEY and NIMBUS_API_SECRET are required outside demo mode");
    if (!mockMode && (!process.env.JWT_SECRET || process.env.JWT_SECRET.length < 32 || process.env.JWT_SECRET === "replace-with-a-long-random-secret"))
