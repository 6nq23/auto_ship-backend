...
export class NimbusClient {
    ...
    async shipMany(orderNumbers, concurrency = 5, onProgress) {
        ...
        const worker = async () => {
            while (cursor < orderNumbers.length) {
                ...
                try {
                    const item = await this.shipOne(orderNumber, controller.signal, onProgress);
                    shipped.push(item);
                    await onProgress?.({ type: "shipped", orderNumber, item });
                }
                catch (error) {
                    const parsed = controller.signal.aborted ? { code: "ORDER_TIMEOUT", error: "NimbusPost did not finish this order within 60 seconds" } : this.describeError(error);
                    const item = { orderNumber, ...parsed };
                    failed.push(item);
                    await onProgress?.({ type: "failed", orderNumber, item });
                }
                finally {
        ...
        if (shipped.length) {
            ...
            try {
                ...
                await onProgress?.({ type: "labels_ready", labelUrl });
            }
            catch (error) {
                const parsed = this.describeError(error);
        ...
        if (pickupScheduled.length) {
            ...
            try {
                ...
                await onProgress?.({ type: "pickup_labels_ready", labelUrl: pickupScheduledLabelUrl });
            }
            catch (error) {
                const parsed = this.describeError(error);
                await onProgress?.({ type: "pickup_labels_failed", error: parsed.error });
            }
        }
    ...
    async generateLabels(orderIds) {
        if (!orderIds.length)
            throw new AppError("NO_LABEL_ORDERS", "This batch has no shipped orders to print");
        return this.labels(orderIds);
    }
    async lookupOrder(orderNumber, signal = AbortSignal.timeout(20_000)) {
        const normalized = normalizeOrderNumber(orderNumber);
        if (!normalized)
            throw new AppError("INVALID_ORDER_NUMBER", "Enter an order number like #RBD5001");
        return this.resolveOrder(normalized, signal);
    }
    async getOrdersByPhone(phone, signal = AbortSignal.timeout(20_000)) {
        ...
        for (let page = 1; page <= this.config.maxPages; page++) {
            ...
            if (phoneFields > 0 && readablePhones === 0)
                break;
            if ((totalPages !== undefined && page >= totalPages) || response.data.length < 100)
                break;
        }
    ...
    async track(awb) {
        if (this.config.mockMode)
            return { orderStatus: "booked", shipment: { awb, courierName: "Delhivery Surface", edd: new Date(Date.now() + 3 * 86_400_000).toISOString() }, latest: { shipStatus: "in transit", eventTime: new Date().toISOString(), location: "Bengaluru Hub", message: "In transit" } };
        return (await this.request(`/v2/tracking/${encodeURIComponent(awb)}`)).data;
    }
    async getNdr(awb) {
        if (this.config.mockMode)
            return { awb, attempt_count: 1, last_attempt_date: new Date().toISOString(), remarks: "Customer not available", available_actions: ["reattempt", "rto"] };
        for (let page = 1; page <= this.config.maxPages; page++) {
            const response = await this.request(`/v2/ndr?limit=100&page=${page}`);
    ...
    async replacePendingOrderAddress(orderNumber, address) {
        ...
        if (!body.order_number || !body.order_type || !body.payment_mode || !body.warehouse_id || !Array.isArray(body.items) || !body.items.length || !body.package || !/^\d{6}$/.test(String(body.shipping_address.pincode)) || !/^\d{10}$/.test(String(body.shipping_address.phone)))
            throw new AppError("RECREATE_DATA_MISSING", "NimbusPost did not return enough order data to recreate this shipment safely");
        ...
        let replacement;
        try {
            replacement = await this.request("/v2/orders", { method: "POST", body: JSON.stringify(body) });
        }
        catch (replacementError) {
            if (!this.isDefinitiveCreateRejection(replacementError))
                throw new AppError("REPLACEMENT_STATUS_UNKNOWN", `NimbusPost did not confirm whether the replacement order was created. AutoShip did not create another order; manual reconciliation is required. ${this.describeError(replacementError).error}`);
            try {
                const restored = await this.request("/v2/orders", { method: "POST", body: JSON.stringify(originalBody) });
    ...
    async shipOne(orderNumber, signal, onProgress) {
        if (this.config.mockMode)
            return this.mockShip(orderNumber, onProgress);
        ...
        for (let index = 0; index < COURIER_PRIORITY.length; index++) {
            ...
            await onProgress?.({ type: "courier_attempt", orderNumber, priority: index + 1, total: COURIER_PRIORITY.length, courierId: courier.courierId, courierName: courier.name, roleId: courier.roleId });
            try {
                const response = await this.request("/v2/shipments/book", { method: "POST", body: JSON.stringify({ order_id: order.order_id, courier_id: courier.courierId }) }, 0, signal);
                ...
                if (!(error instanceof AppError))
                    throw new AppError("BOOKING_STATUS_UNKNOWN", `Could not confirm whether ${courier.name} booked the shipment. No further courier was attempted.`);
                const parsed = this.describeError(error);
                rejected.push(`${courier.name}: ${parsed.error}`);
                await onProgress?.({ type: "courier_rejected", orderNumber, priority: index + 1, courierName: courier.name, code: parsed.code, error: parsed.error });
                if (["UNAUTHORIZED", "FORBIDDEN", "RATE_LIMITED", "TOO_MANY_REQUESTS", "HTTP_401", "HTTP_403", "HTTP_429"].includes(parsed.code))
                    throw error;
            }
        }
    ...
    async resolveOrder(orderNumber, signal) {
        ...
        if (cached)
            return this.getOrder(cached, signal);
        ...
        if (!match)
            throw new AppError("NOT_FOUND", `Order ${orderNumber} was not found in NimbusPost`);
        await this.cache.cacheOrder(orderNumber, match.order_id);
        return match;
    }
    listOrders(query, signal) { return this.request(`/v2/orders?${new URLSearchParams(query)}`, {}, 0, signal); }
    async getOrder(orderId, signal) { return (await this.request(`/v2/orders/${encodeURIComponent(orderId)}`, {}, 0, signal)).data; }
    async getOrderDetails(orderId) { return (await this.request(`/v2/orders/${encodeURIComponent(orderId)}`)).data; }
    async labels(ids) {
        if (this.config.mockMode)
            return `/demo-labels?ids=${encodeURIComponent(ids.join(","))}`;
        try {
            const response = await this.request("/v2/shipments/labels", { method: "POST", body: JSON.stringify({ ids }) });
            return response.data.url;
        }
        catch (error) {
            if (!(error instanceof AppError) || !/\border_ids\b/i.test(error.message))
                throw error;
            const response = await this.request("/v2/shipments/labels", { method: "POST", body: JSON.stringify({ order_ids: ids }) });
            return response.data.url;
        }
    }
    async request(path, init = {}, attempt = 0, overallSignal) {
        const controller = new AbortController();
        ...
        let response;
        try {
            response = await fetch(`${this.config.apiUrl}${path}`, { ...init, headers: { Accept: "application/json", "Content-Type": "application/json", "x-api-key": this.config.apiKey, "x-api-secret": this.config.apiSecret, ...init.headers }, signal: controller.signal });
        }
        finally {
            clearTimeout(timeout);
        ...
        if (response.status === 429 && attempt < 2) {
            const wait = Math.min(Number(response.headers.get("retry-after") || 1) * 1000, 5000);
            await new Promise((resolve) => setTimeout(resolve, wait));
            return this.request(path, init, attempt + 1, overallSignal);
        }
        const body = await response.json().catch(() => ({}));
        if (!response.ok)
            throw new AppError(body.error?.code || `HTTP_${response.status}`, body.error?.detail || "NimbusPost rejected the request");
        return body;
    }
    async mockShip(orderNumber, onProgress) {
        await new Promise((resolve) => setTimeout(resolve, 20));
        const suffix = Number(orderNumber.replace(/\D/g, ""));
        if (orderNumber.endsWith("30"))
            throw new AppError("NOT_FOUND", `Order ${orderNumber} was not found in NimbusPost`);
        if (orderNumber.endsWith("44"))
            return { orderNumber, orderId: `ORD-${suffix}`, awb: `AWB${240000000 + suffix}`, courier: "Bluedart Brand", cost: 92, alreadyBooked: true };
        for (let index = 0; index < COURIER_PRIORITY.length; index++) {
            const courier = COURIER_PRIORITY[index];
            await onProgress?.({ type: "courier_attempt", orderNumber, priority: index + 1, total: COURIER_PRIORITY.length, courierId: courier.courierId, courierName: courier.name, roleId: courier.roleId });
            await new Promise((resolve) => setTimeout(resolve, 10));
            if (orderNumber.endsWith("35") || (orderNumber.endsWith("24") && index === 0)) {
                const error = "No courier service is available for the destination pincode";
                await onProgress?.({ type: "courier_rejected", orderNumber, priority: index + 1, courierName: courier.name, code: "NO_SERVICEABLE_COURIER", error });
                continue;
            }
    ...
    describeError(error) { if (error instanceof AppError)
        return { code: error.code, error: error.message }; return { code: "NETWORK_ERROR", error: "NimbusPost could not be reached. Try this order again." }; }
    isDefinitiveCreateRejection(error) { return error instanceof AppError && ["VALIDATION_FAILED", "BAD_REQUEST", "INVALID_REQUEST", "DUPLICATE_ORDER", "DUPLICATE_ORDER_NUMBER", "HTTP_400", "HTTP_409", "HTTP_422"].includes(error.code); }
}
